...
export const onAddMessage = () => {
    config.dialogs.messageData.push({message: config.dialogs.newMessage, isFriend: false})
    config.dialogs.newMessage = ''
    rerenderTree(config)
}
export const onAddPost = () => {
    config.profile.postData.push({postText: config.profile.newPost, likeCount: 0})
    config.profile.newPost = ''
    rerenderTree(config)
}

export const onChangeNewMessage = (message) => {
    config.dialogs.newMessage = message
    rerenderTree(config)
}
export const onChangeNewPost = (post) => {
    config.profile.newPost = post
    rerenderTree(config)
}
